...
use robopoker::{cards::{hand::Hand, isomorphism::Isomorphism, observation::Observation}, clustering::{abstraction::Abstraction, abstractor::Abstractor, histogram::Histogram as CardsHistogram}};

fn main() {
    logging();
    let a = Isomorphism::from(Observation::from((
        Hand::from("As Ah"),
        Hand::from("9d 7h 8c Ac"),
    )));

    let abstractor = Abstractor::default();

    let histogram = abstractor.projection(&a);
...
fn plot_histogram(isomorphism: &Isomorphism, histogram: &CardsHistogram) {
    ...
    let path = format!("images/{}_equity_2.png", isomorphism);
    ...
    let mut chart = ChartBuilder::on(&root_drawing_area)
        .caption(format!("Equity Histogram for {}", isomorphism), ("sans-serif", 30).into_font())
        .margin(10)
